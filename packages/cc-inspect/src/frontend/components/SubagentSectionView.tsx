...
function AgentLabel({
	...
	const subtitle = [modelLabel, totalTokens != null ? formatTokenCount(totalTokens) : null]
		.filter(Boolean)
		.join(" · ")
	return (
...
export function SubagentSectionView({
	...
	if (!isComplete) {
		...
		return (
			<div
				data-agent-section={section.agentId}
				className={`ml-3 pl-4 border rounded-xl py-3 pr-4 ${colors.bg} ${colors.border} animate-pulse`}
			>
				<button
					type="button"
	...
	return (
		<div
			data-agent-section={section.agentId}
			className={`ml-3 pl-4 border rounded-xl py-3 pr-4 ${colors.bg} ${colors.border}`}
		>
