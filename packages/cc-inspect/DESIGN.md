...
- **When an event is selected**: Shows full details including:
  ...
  - A **raw/rendered toggle** in the header switches markdown content between rendered markdown and raw pre-formatted text. Affects user messages, assistant messages, agent spawn prompts, summaries, and plan handoff plans. The preference is persisted in `localStorage` under `cc-inspect-ui`. When active, the toggle button shows an amber highlight.
